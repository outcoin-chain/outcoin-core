...
The Outcoin Core daemon can be configured to act as a trusted "border
router", implementing the outcoin wire protocol and relay, making
...
The ZeroMQ feature in Outcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building outcoind:
...
For instance:
    ...
    $ outcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/outcoind.tx.raw \
...
These options can also be provided in outcoin.conf.
...
From the perspective of outcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into outcoind directly. Furthermore, no information is
...
using. Outcoind appends an up-counting sequence number to each
