...
std::vector<fs::path> ListWalletDir()
{
    ...
    for (auto it = fs::recursive_directory_iterator(wallet_dir, ec); it != fs::recursive_directory_iterator(); it.increment(ec)) {
        ...
        try {
            ...
            } else if (it->symlink_status().type() == fs::regular_file && ExistsBerkeleyDatabase(it->path())) {
                // Check if this is a top-level file by comparing the parent path
                if (it->path().parent_path() == wallet_dir) {
