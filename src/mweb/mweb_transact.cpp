...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    static auto is_out = [](const CInputCoin& input) { return !input.IsMWEB(); };
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_out)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple OUT-to-OUT transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::OUT_TO_OUT;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::OUT_TO_OUT) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    // Lookup the change paid on the OUT side
    CAmount out_change = 0;
    if (new_tx.change_position != -1) {
        assert(new_tx.tx.vout.size() > (size_t)new_tx.change_position);
        out_change = new_tx.tx.vout[new_tx.change_position].nValue;
    }

    // Calculate pegin_amount
    boost::optional<CAmount> pegin_amount = boost::none;
    CAmount out_input_amount = std::accumulate(
    ...
    if (out_input_amount > 0) {
        const CAmount out_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(out_fee <= out_input_amount);
        pegin_amount = (out_input_amount - (out_fee + out_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, out_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& out_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + out_change);
    if (change_amount < 0) {
